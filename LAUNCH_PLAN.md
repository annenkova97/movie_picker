...
- [ ] **Step 1 — Sentry**
  - [ ] Backend: `sentry-sdk[fastapi]`, DSN в env
  - [ ] Frontend: `@sentry/react`, DSN в env
  - [ ] Тестовая ошибка с обеих сторон, проверить что долетают
- [x] **Step 2 — Rate limiting** (`slowapi`; реализация в `backend/rate_limit.py`)
  ...
  - [ ] `/movies/add` — мягко (ещё НЕ покрыт; добавить per-user лимит отдельно)
...
- [ ] **Step 3 — Error boundaries в React** (fallback UI вместо белого экрана)
- [ ] **Step 4 — Бэкапы**
  - [ ] GitHub Action: `pg_dump` раз в день
  - [ ] Дамп шифруется (gpg) и коммитится в приватный backup-репо
  - [ ] Зафиксировать procedure restore
- [ ] **Step 5 — Мини Privacy Policy**
  - [ ] Одна страница: что храним (email+пароль+фильмы), зачем, какие внешние сервисы (TMDB/OMDB/Sentry/LLM-провайдер), как удалить аккаунт (написать мне)
  - [ ] Страница `/privacy` на фронте, ссылка в футере
